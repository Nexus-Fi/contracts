...
TXHASH="$(nibid tx wasm store artifacts/nexus_stnibi_token_test.wasm \
    --from $FROM \
    --gas auto \
    --gas-adjustment 1.5 \
...
echo $TXHASH
sleep 6
nibid q tx $TXHASH >nexus_stnibi_token_test.json
CODE_ID="$(cat nexus_stnibi_token_test.json | jq -r '.logs[0].events[1].attributes[1].value')"
echo 'CODE_ID:'
echo $CODE_ID
...
TXHASH_INIT="$(nibid tx wasm instantiate $CODE_ID \
    "$(cat initiate_token_stnibi.json)" \
    --admin "$FROM" \
    --label "contract" \
    --from $FROM \
    --gas auto \
...
echo $TXHASH_INIT
sleep 6
nibid q tx $TXHASH_INIT >initiate_token_stnibi.init.json

CONTRACT_ADDRESS="$(cat initiate_token_stnibi.init.json | jq -r '.logs[0].events[1].attributes[0].value')"
